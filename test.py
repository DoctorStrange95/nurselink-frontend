...
try:
    req = urllib.request.Request('https://nurselink-frontend-8r6i.onrender.com/api/auth/login', 
        data=json.dumps({'email':'arjun@nurselink.in', 'password':'password'}).encode('utf-8'),
        headers={'Content-Type': 'application/json'})
    res = urllib.request.urlopen(req, context=ctx)
    token = json.loads(res.read())['token']

    req2 = urllib.request.Request('https://nurselink-frontend-8r6i.onrender.com/api/patients', 
